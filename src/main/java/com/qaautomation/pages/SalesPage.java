...
public boolean isSaleDeleted(String saleName) {
    try {
        // Create a locator for the specific row containing the deleted plant name
        By deletedRowLocator = By.xpath("//table/tbody/tr[td[contains(text(),'" + saleName + "')]]");
        
        // Wait specifically for that row to vanish from the DOM
        return wait.until(ExpectedConditions.invisibilityOfElementLocated(deletedRowLocator));
    } catch (Exception e) {
        // If an exception occurs, re-check the list manually as a fallback
        List<WebElement> rows = driver.findElements(By.xpath("//table/tbody/tr[td[contains(text(),'" + saleName + "')]]"));
        return rows.isEmpty();
    }
}
 ...
 public boolean isSortedAscending(List<String> values) {
    ...
    // Use CASE_INSENSITIVE_ORDER to match standard UI sorting behavior
...
public boolean isSortedDescending(List<String> values) {
    ...
    List<String> sorted = new ArrayList<>(values);
    // Sort reverse case-insensitively
...
private By nextPageLink = By.xpath("//li[not(contains(@class,'disabled'))]/a[contains(text(),'Next')]");
private By pageTwoLink = By.xpath("//li/a[text()='2']");
private By activePageItem = By.cssSelector("li.page-item.active a.page-link");

public void scrollToBottom() {
    // We use JavaScript because standard Selenium actions sometimes 
    // struggle with scrolling to the very end of dynamic tables.
...
public void clickNextPage() {
    ...
    WebElement nextBtn = wait.until(ExpectedConditions.elementToBeClickable(nextPageLink));
    
    ((JavascriptExecutor) driver).executeScript("arguments[0].click();", nextBtn);
    
    wait.until(ExpectedConditions.textToBePresentInElementLocated(activePageItem, "2"));
}
...
public String getActivePageText() {
    ...
    return wait.until(ExpectedConditions.visibilityOfElementLocated(activePageItem)).getText();
}
}
