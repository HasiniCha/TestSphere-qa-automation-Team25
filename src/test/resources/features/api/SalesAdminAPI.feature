...
Feature: Sales API Functionality
  ...
  Scenario: Sell plant with valid request
    ...
    When Admin sends a POST request to "/sales/plant/{plantId}" for plant 5 with quantity 2
  ...
  Scenario Outline: Sell plant with invalid quantity
    Given Admin is authenticated for API
    When Admin sends a POST request to "/sales/plant/{plantId}" for plant 5 with quantity <qty>
 ...
 Scenario: Delete sale by ID
  Given Admin is authenticated for API
  And a sale with ID 37 exists in the system
  When Admin sends a DELETE request to "/sales/{id}" for sale ID 37
