...
Feature: Sales API Functionality
  ...
  Scenario: Sell plant with valid request
    ...
    And a plant with ID 2 exists and has valid stock
    When Admin sends a POST request to "/sales/plant/{plantId}" for plant 2 with quantity 5
  ...
  Scenario Outline: Sell plant with invalid quantity
    Given Admin is authenticated for API
    When Admin sends a POST request to "/sales/plant/{plantId}" for plant 2 with quantity <qty>
  ...
  Scenario: Delete sale by ID
    Given Admin is authenticated for API
    When Admin sends a DELETE request to "/sales/{id}" for sale ID 27
